...
const Products = () => {
  ...
  useEffect(() => {
    // const storedProducts = localStorage.removeItem("products");
  ...
  const handleViewDetails = (product) => {
    ...
    const isViewed = recentViews.find((item) => item.id === product.id);
    if (!isViewed) {
      
      const updatedViews = [...recentViews, product];
  ...
  const handlesorting = (order) => {
    const sortProducts = [...products].sort((a, b) => {
      ...
      if (order === "highRating") return b.rating - a.rating;
      return 0;
    });
  ...
  const filteredProducts = products.filter((product) => {
    const matchesCategory = selectedCategory ? product.category.toLowerCase().trim() === selectedCategory.toLowerCase().trim() : true;
  ...
  return (
    <div id="products" className="px-4 sm:px-6 lg:px-8">
      ...
      <Filter onSortChange={handlesorting} />
        ...
        {filteredProducts.map((product) => (
          <div key={product.id} className="flex flex-col bg-pink-100 p-4 rounded-lg shadow-md hover:bg-pink-200 transition duration-300">
            ...
            <Link href={`/ProductsDetails/${product.id}`}>
              <button className="bg-pink-600 text-white px-4 py-2 rounded-md mt-2 hover:bg-pink-700 transition duration-200 w-full" onClick={() => handleViewDetails(product)}>View Details</button>
            </Link>

              <button onClick={() => addToWishlist(product)} className="border border-pink-500 text-pink-600 px-2 py-1 sm:px-4 sm:py-2 rounded-md mt-2 sm:mt-0 sm:ml-4 hover:bg-pink-600 hover:text-white transition duration-200 w-full sm:w-auto"> ❤️ </button>
            </div>
          </div>
        ))}
